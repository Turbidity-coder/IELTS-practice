...
pub const LEARNING_OBSERVATION_PROJECTOR_VERSION: i64 = 1;
...
pub fn learning_observations_rebuild(
    ...
    let report = learning_observations_rebuild_in_transaction(&tx)?;
    tx.commit()?;
    Ok(report)
}
...
pub(crate) fn learning_observations_rebuild_in_transaction(
    ...
    conn.execute(
        "DELETE FROM learner_observations WHERE projector_key = ?1 AND projector_version = ?2",
        params![
...
fn project_reading_repeats(events: &[ReadingQuestionEvent]) -> Vec<ObservationCandidate> {
    ...
    for ((_user_id, asset_id, question_id), mut observations) in grouped {
        ...
        for pair in observations.windows(2) {
            let previous = &pair[0];
            let current = &pair[1];
            let state = question_transition_state(previous.is_correct, current.is_correct);
...
fn load_stored_records(conn: &Connection) -> DbResult<Vec<ObservationRecord>> {
    let mut statement = conn.prepare(
        "SELECT id, user_id, observation_type, namespace, scope_kind, scope_key, polarity,
         ...
         WHERE projector_key = ?1 AND projector_version = ?2
         ORDER BY id",
    )?;
    ...
    for record in &mut records {
        ...
        let rows = evidence.query_map(params![record.id], |row| {
        ...
        record.evidence = rows.collect::<Result<Vec<_>, _>>()?;
    }
    Ok(records)
}
...
fn validate_reading_attempt_payload(
    ...
    validate_nonnegative_finite(payload.score_value, "scoreValue")?;
...
fn validate_writing_score(score: &WritingScorePayload) -> Result<(), String> {
    for (field, value) in [
        ...
        if !value.is_finite() {
            return Err(format!("writing score field is not finite: {field}"));
        }
    }
    Ok(())
}
